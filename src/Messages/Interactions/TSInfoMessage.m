...
- (NSString *)previewTextWithTransaction:(SDSAnyReadTransaction *)transaction
{
    switch (_messageType) {
        ...
        case TSInfoMessageAIBotStateChange: {
            NSString *key = [[NSString alloc] initWithFormat:@"AIBotStateManage_%@", self.thread.uniqueId];
            BOOL state = [[NSUserDefaults standardUserDefaults] boolForKey:key];
            if (state) {
                return @"AI机器人已开启";
            } else {
                 return @"AI机器人已关闭";
        ...
        case TSInfoMessageHisAIBotStateChange: {
            NSString *key = [[NSString alloc] initWithFormat:@"AIBotStateManage_His_%@", self.thread.uniqueId];
            BOOL state = [[NSUserDefaults standardUserDefaults] boolForKey:key];
            if (state) {
                return @"对方AI机器人已开启";
            } else {
