...
//                                               name: NSNotification.Name(rawValue: kNSNotification_OWSWebSocketStateDidChange),
//                                               object: nil)
        NotificationCenter.default.addObserver(self,
...
//     This should only be accessed on serialQueue.
    ...
    private func tryToResolveDecryptStepPromises() {
        ...
        let hasPendingJobs = self.isDecryptingIncomingMessages()
    ...
    private func isDecryptingIncomingMessages() -> Bool {
        return messageReceiver.hasPendingJobs()
    }
...
//        }
        ...
        let messageFetcherJob = SSKEnvironment.shared.messageFetcherJob

        if messageFetcherJob.shouldUseWebSocket {
            let isWebsocketDrained = (self.socketManager.highestSocketState() == .open &&
                self.socketManager.hasEmptiedInitialQueue())
            guard isWebsocketDrained else {
//                if DebugFlags.isMessageProcessingVerbose {
