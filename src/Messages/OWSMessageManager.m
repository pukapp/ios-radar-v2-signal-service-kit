                                          ...
                                          transaction:(SDSAnyWriteTransaction *)transaction
{
    ...
    if (dataMessage.hasBody) {
        TSIncomingMessage *message = [self createIncomingMessageInThread:thread
                                                      messageDescription:@"训练者发送信息给被训练者"
                                                                envelope:envelope
                                                             dataMessage:dataMessage
                                                         wasReceivedByUD:wasReceivedByUD
                                                             transaction:transaction];
...
- (void)handleAIBeTrainerToTrainerMessageWithEnvelope:(SSKProtoEnvelope *)envelope
                                          dataMessage:(SSKProtoDataMessage *)dataMessage
                                      wasReceivedByUD:(BOOL)wasReceivedByUD
                                          transaction:(SDSAnyWriteTransaction *)transaction
{
    ...
    if (dataMessage.hasBody) {
        TSIncomingMessage *message = [self createIncomingMessageInThread:thread
                                                      messageDescription:@"被训练者发送信息给训练者"
                                                                envelope:envelope
                                                             dataMessage:dataMessage
                                                         wasReceivedByUD:wasReceivedByUD
                                                             transaction:transaction];
...
- (void)handleAIBotMessageWithEnvelope:(SSKProtoEnvelope *)envelope
                           dataMessage:(SSKProtoDataMessage *)dataMessage
                           transaction:(SDSAnyWriteTransaction *)transaction
{
    if (!envelope) {
    ...
    if (dataMessage.hasBody) {
        TSOutgoingMessage *message = [[TSOutgoingMessage alloc] initOutgoingMessageWithTimestamp:NSDate.ows_millisecondTimeStamp inThread:thread messageBody:dataMessage.body attachmentIds:[NSMutableArray new] expiresInSeconds:0 expireStartedAt:0 isVoiceMessage:NO groupMetaMessage:TSGroupMetaMessageUnspecified quotedMessage:nil contactShare:nil linkPreview:nil messageSticker:nil perMessageExpirationDurationSeconds:0];
        [message anyInsertWithTransaction:transaction];
        [message updateWithFakeMessageState:TSOutgoingMessageStateSent transaction:transaction];
    }
}

- (void)handleAIBotStateUpdateMessageWithEnvelope:(SSKProtoEnvelope *)envelope
                                      dataMessage:(SSKProtoDataMessage *)dataMessage
                                      transaction:(SDSAnyWriteTransaction *)transaction
{
    if (!envelope) {
    ...
    if (dataMessage.hasAiBotState) {
        BOOL state = dataMessage.aiBotState;
        NSString *key = [[NSString alloc] initWithFormat:@"AIBotStateManage_His_%@", thread.uniqueId];
        [NSUserDefaults.standardUserDefaults setBool:state forKey:key];
        NSString *str = [[NSString alloc] initWithFormat:@"对方机器人状态已更改(%@)", state ? @"YES" : @"NO"];
        TSInfoMessage *msg = [[TSInfoMessage alloc] initWithTimestamp:NSDate.ows_millisecondTimeStamp inThread:thread messageType:TSInfoMessageHisAIBotStateChange customMessage:str];
        [msg anyInsertWithTransaction:transaction];
    }
}
